...
- **Deploy artifact.** `.hodu` is a self-describing container: named params, non-learnable buffers (e.g. BatchNorm running stats), and optimizer state -- so save/load round-trips eval-mode behavior and resumes training. `save_runnable` additionally writes the forward graph and `load_runnable` rebuilds and runs it, so the artifact runs from the file alone.
